...
def login():
    if request.method == 'POST':
        ...
        sql_query_authentication = f"SELECT * FROM users WHERE username = '{username}' AND password = '{password}'"
        db_conn = get_db_connection()
        cursor = db_conn.cursor()

        try:
            cursor.execute(sql_query_authentication)
...
def search():
    if 'username' in session:
        ...
        if request.method == 'POST':
            ...
            try:
                sql_query = f"SELECT first_name FROM actor WHERE first_name = '{search_term}'"
                cursor.execute(sql_query)
